...
// import { useToast } from '@/components/ui/use-toast'; // dihapus
import { toast } from "sonner"; // <-- pakai sonner
import * as z from "zod";
import { useState, useEffect } from "react";
import { RoomDetailsForm } from "./RoomDetailsForm";
import { useGetListingsFacilities } from "@/components/listings/hooks/useGetListings";
import { listingRoomtypeFormSchema } from "@/components/listings/schemas/ListingFormSchema";
...
export const HotelRoomTypePopup = () => {
  ...
  const onSetFiles = (event: any) => {
    try {
      ...
      toast.error(error.message); // <-- menggunakan sonner
      reset();
    }
  };
  ...
  const onSubmit = async (
    ...
    reset();
    toast.success("Room type added successfully!"); // <-- toast sukses
  };
  ...
  const handleDelete = (index: number) => {
    ...
    toast.info("Room type deleted"); // <-- toast info
  };
